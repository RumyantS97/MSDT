...
def write_data(post_data, url):
    global change
    print('\n---------------- CONFIGURATION ----------------')
    print(json.dumps(post_data, sort_keys=True, indent=4,
                     separators=(',', ': ')))
    print('\n---------------------------------------------')
    choice = input(
        '\nWould you want to push the above config to the firewall? [yes/no]').lower()
    if choice == 'yes':
        req = request.Request(url, data=json.dumps(post_data).encode('utf-8'),
                              headers=headers)
        base64string = base64.encodebytes(
            f'{username}:{password}'.encode()).decode('utf-8').replace('\n',
                                                                       '')
...
def main(argv):
    ...
    if change == 1:
        choice = input(
            '\nThe device configuration has been changed by the script. '
            'Save to NVRAM? [yes/no]').lower()
        if choice in ('', 'yes'):
