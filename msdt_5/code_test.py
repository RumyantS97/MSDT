...
def test_order_add_product_insufficient_stock():
    ...
    try:
        ...
        assert str(e) == "Insufficient stock for Laptop"
    else:
        assert False, "Expected ValueError to be raised"
    
    assert len(order.items) == 0

def test_shop_process_order_complex():
    shop = Shop()
    product1 = Product("Laptop", 999.99, 5)
    product2 = Product("Mouse", 19.99, 10)
    ...
    assert product3.stock == 6
    
    total = order.calculate_total()
    assert total == (2 * 999.99 + 3 * 19.99 + 1 * 49.99)
    
    assert order.processed is True