...
if b > 50:
    ...
    elif b == 50: print(f"Число {b} равно 50")
else:
...
class Car:
    def __init__(self, brand, color): self.brand = brand; self.color = color
...
for item in data:
    if item > 5:
        result.append(item * 2) elif item == 5:
...
def long_calculation(x, y, z): res = 0
...
def recursive_example(n): if


n > 0: print(n)
recursive_example(n - 1)
else: print("Базовый случай")
...
else: print("Строка не содержит букву 'a'")


class Building:
    def __init__(self, floors, color): self.floors = floors;self.color = color

    def describe(self): print(f"Здание имеет {self.floors} этажей и {self.color} цвет")

    def build(self): print(f"Строим здание с {self.floors} этажами")





def demolish(self): print("Здание снесено")
...
else:print("y не больше z")
else:print("x не больше y")
...
for n in numbers: if
n % 2 == 0: print(f"Число {n} чётное")
else:print(f"Число {n} нечётное")
