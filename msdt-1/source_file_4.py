...
while flag:
    ...
    if symbol == "1":
        ...
        print("\nПрограмма для определения попадания в мишень выстрела"
        ...
        if x > 1:
            while abs(sum - presum) > e:
                ...
                quan += 1
                print(f"n = {quan}")
                sum += math.pow(-1, quan + 1) / ((2 * quan + 1)
                        * math.pow(x, 2 * quan + 1))
                print(f"Сумма: {sum}\n")
        ...
        print(f"\nВы ввели строку символов \"{symbol}\", "
              + f"возможно, это было случайно, попробуйте снова!")
