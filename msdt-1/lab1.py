...
print("99% доверительный интервал для среднего"
      " времени обработки одной детали:", interval)
...
if p_value < alpha:
    print("Отвергаем нулевую гипотезу. Станки не обладают"
...
# Проверка статистической значимости
if p_value < alpha:
    print("Отвергаем нулевую гипотезу. Существует существенное"
          " различие средней себестоимости "
          "единицы продукции на предприятиях.")
else:
...
s_x = (sum([n * ((x - mean_x) ** 2) for x, n in zip(x_values, n_x)])
       / (sum(n_x) - 1)) ** 0.5
s_y = (sum([n * ((y - mean_y) ** 2) for y, n in zip(y_values, n_y)])
       / (sum(n_y) - 1)) ** 0.5

# Расчет t-статистики и p-значения
t_statistic, p_value = st.ttest_ind_from_stats(mean_x, s_x,
                                               sum(n_x), mean_y, s_y, sum(n_y))
...
if p_value < alpha:
    print("Отвергаем нулевую гипотезу. Существует существенное различие"
          " средних значений дебиторской задолженноститв группах.")
else:
    print("Принимаем нулевую гипотезу. Различие средних значений"
          " дебиторской задолженности в группах несущественно.")
...
if p_value < alpha:
    print("Отвергаем нулевую гипотезу. Существует существенное различие средних"
          " значений средней выручки в группах.")
else:
    print("Принимаем нулевую гипотезу. Различие средних значений средней"
          " выручки в группах несущественно.")
