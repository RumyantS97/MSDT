...
class WindowUI:
    def __init__(self):
        ...
        buttonGo1 = tk.Button(self.tab1, text="Go!",
                              command=lambda: [self.newCreateFolders(textBox1.get("1.0", 'end-1c'), v1.get())])
        ...
        incrementButton1 = tk.Checkbutton(self.tab1, text="Increment mode", variable=self.incrementVariable, onvalue=1,
                                          offvalue=0, command=lambda: [self.incrementSelector(self.tab1)])
        ...
        buttonGo2 = tk.Button(self.tab2, text="Go!", command=lambda: [
            self.newRemoveFolders(testBox2.get("1.0", 'end-1c'), v4.get(), entryBox1.get())])
        buttonGo2.pack(side="bottom", pady=10)

        incrementButton2 = tk.Checkbutton(self.tab2, text="Increment mode", variable=self.incrementVariable, onvalue=1,
                                          offvalue=0, command=lambda: [self.incrementSelector(self.tab2)])
        ...
        buttonGo3 = tk.Button(self.tab3, text="Go!", command=lambda: [
            self.newModifyFolders(entryBox2.get(), v2.get(), entryBox3.get(), v3.get())])
    ...
    def newRemoveFolders(self, entryGet="", modeSelected=0, startsEndsWith=""):
        ...
        p = threading.Thread(
            target=self.execution.removeFolders(entryGet, modeSelected, startsEndsWith, incrementValue, incrementStart))
        p.start()
        self.logs.configure(state="normal")
        self.logs.insert("1.0", f"---------------------------------\n{self.execution.functionsOutput}")
        self.logs.configure(state="disabled")
