...
def open_operation_window(operation):
  ...
  table_combo = ttk.Combobox(operation_window, values = ["books", "contract", "creative_groups", 
                             "customers", "orders", "published_books", "writers"])
  table_combo.grid(row = 0, column = 1, padx = 5, pady = 5)

  tk.Button(operation_window, text = "Загрузить данные", command = fetch_and_display_data).grid(row = 1, 
                                               column = 0 , 
                                               columnspan = 2, 
                                               pady = 5)
  ...
  if operation == "INSERT":
    tk.Label(operation_window, text = "Введите значения по образцу в таблице через запятую:").grid(row = 5, 
                                                   column = 0, 
                                                   padx = 5, 
                                                   pady = 5)
    ...
    tk.Button(operation_window, text = "Выполнить", command = perform_insert).grid(row = 6, 
                                          column = 0, 
                                          columnspan = 2, 
                                          pady = 10)

  elif operation == "UPDATE":
    tk.Label(operation_window, text = "Введите SET выражение (например, column = value):").grid(row = 4, 
                                                column = 0, 
                                                padx = 5, 
                                                pady = 5)
    ...
    tk.Button(operation_window, text = "Выполнить", command = perform_update).grid(row = 6, 
                                          column = 0, 
                                          columnspan = 2, 
                                          pady = 10)
    ...
    tk.Button(operation_window, text = "Выполнить", command = perform_delete).grid(row = 5, 
                                          column = 0, 
                                          columnspan = 2, 
                                          pady = 10)
...
def open_report_window():
  ...
  def generate_report():
    ...
    query = f"SELECT customer as Заказчик, books.name as Книга, cost as Стоимость, price as Цена, \
         count as Число, (price - cost) * count as Прибыль from books join orders \
         on (books.code = orders.code) where year(completionDate) = {year} union \
         SELECT 'Итого', '', '', '', '', sum((price - cost) * count) from books join orders \
         on (books.code = orders.code) where year(completionDate) = {year};"
...
btn_books = tk.Button(root, text = "Таблица 'Книги'", command = lambda: 
           execute_query("select * from books"), wraplength = 200)
btn_books.grid(row = 5, column = 0, padx = 5, pady = 5)

btn_contract = tk.Button(root, text = "Таблица 'Контракты'", command = lambda: 
             execute_query("select * from contract"), wraplength = 200)
btn_contract.grid(row = 5, column = 1, padx = 5, pady = 5)

btn_creative_groups = tk.Button(root, text = "Таблица 'Творческие группы'", command = lambda: 
                execute_query("select * from creative_groups"), wraplength = 200)
btn_creative_groups.grid(row = 5, column = 2, padx = 5, pady = 5)

btn_customers = tk.Button(root, text = "Таблица 'Заказчики'", command = lambda: 
             execute_query("select * from customers"), wraplength = 200)
...
btn_published_books = tk.Button(root, text = "Таблица 'Опубликованные книги'", command = lambda: 
                execute_query("select * from published_books"), wraplength = 200)
btn_published_books.grid(row = 6, column = 1, padx = 5, pady = 5)

btn_writers = tk.Button(root, text = "Таблица 'Писатели'", command = lambda: 
            execute_query("select * from writers"), wraplength = 200)
