...
def open_operation_window(operation):
  ...
  def perform_insert():
    ...
    if selected_table:
      ...
      error_label.config(text = message, fg = "green" 
                         if "успешно" in message else "red")
      error_label.update()
  ...
  def perform_update():
    ...
    if selected_table:
      ...
      if not condition:
        error_label.config(text = "Ошибка: условие \
                           WHERE не может быть пустым", fg = "red")
        return

      # Формируем запрос
      query = f"UPDATE {selected_table} SET {formatted_set_clause} \
               WHERE {formatted_condition};"

      # Выполняем запрос
      message = execute_query(query)
      
      # Отображаем сообщение об ошибке или успехе
      error_label.config(text = message, fg = "green" if "успешно" 
                         in message else "red")
      error_label.update()
  ...
  tk.Label(operation_window, text = "Выберите таблицу:").grid(row = 0, 
                                                              column = 0, 
                                                              padx = 5, 
                                                              pady = 5)
  table_combo = ttk.Combobox(operation_window, values = ["books", 
                                                         "contract", 
                                                         "creative_groups", 
                                                         "customers", 
                                                         "orders", 
  ...
  tk.Button(operation_window, text = "Загрузить данные", 
            command = fetch_and_display_data).grid(row = 1, 
                                                   column = 0 , 
                                                   columnspan = 2, 
                                                   pady = 5)
  ...
  if operation == "INSERT":
    tk.Label(operation_window, text = "Введите значения \
                                       по образцу в таблице \
                                       через запятую:").grid(row = 5, 
                                                             column = 0, 
                                                             padx = 5, 
                                                             pady = 5)
    ...
    tk.Button(operation_window, text = "Выполнить", 
              command = perform_insert).grid(row = 6, 
                                             column = 0, 
                                             columnspan = 2, 
                                             pady = 10)

  elif operation == "UPDATE":
    tk.Label(operation_window, text = "Введите SET выражение \
                                       (например, column = value):").grid(
                                                row = 4, 
                                                column = 0, 
                                                padx = 5, 
                                                pady = 5)
    ...
    tk.Label(operation_window, text = "Введите условие \
                                       (например, id = 1):").grid(row = 5,
                                                                  column = 0,
                                                                  padx = 5, 
                                                                  pady = 5)
    ...
    tk.Button(operation_window, text = "Выполнить", command = 
              perform_update).grid(row = 6, 
                                   column = 0, 
                                   columnspan = 2, 
                                   pady = 10)
    ...
    tk.Label(
             operation_window, 
             text = "Введите условие (например, id = 1):"
            ).grid(row = 4, column = 0, padx = 5, pady = 5)
    condition_entry = tk.Entry(operation_window, width = 50)
    condition_entry.grid(row = 4, column = 1, padx = 5, pady = 5)

    tk.Button(operation_window, text = "Выполнить", 
              command = perform_delete).grid(row = 5, 
                                             column = 0, 
                                             columnspan = 2, 
                                             pady = 10)
...
def generate_pdf(data, columns, filename, year):
  ...
  pdf.cell(0, 10, "Задание 26. Проект ИЗДАТЕЛЬСКИЙ ЦЕНТР", 
           ln = True, align = 'C')
  pdf.cell(0, 10, "Прибыль от продаж книг издательского центра 'Печать'",
            ln = True, align = 'C')
...
def get_years():
  query = "SELECT DISTINCT YEAR(completionDate) \
           AS year FROM orders ORDER BY year"
...
def open_report_window():
  ...
  def generate_report():
    ...
    query = f"SELECT customer as Заказчик, books.name as Книга, \
              cost as Стоимость, price as Цена, count as Число, \
              (price - cost) * count as Прибыль from books join \
              orders on (books.code = orders.code) where \
              year(completionDate) = {year} union SELECT 'Итого', \
              '', '', '', '', sum((price - cost) * count) from books \
              join orders on (books.code = orders.code) \
              where year(completionDate) = {year};"
  ...
  tk.Button(report_window, text = "Сгенерировать", 
...
btn_insert = tk.Button(root, text = "ВСТАВКА", 
                       command = lambda: open_operation_window("INSERT"))
btn_insert.grid(row = 2, column = 0, padx = 5, pady = 5)

btn_update = tk.Button(root, text = "ИЗМЕНЕНИЕ", 
                       command = lambda: open_operation_window("UPDATE"))
btn_update.grid(row = 2, column = 1, padx = 5, pady = 5)

btn_delete = tk.Button(root, text = "УДАЛЕНИЕ",
                        command = lambda: open_operation_window("DELETE"))
btn_delete.grid(row = 2, column = 2, padx = 5, pady = 5)

btn_otchet = tk.Button(root, text = "Сгенерировать отчет", 
...
btn_books = tk.Button(root, text = "Таблица 'Книги'", command = lambda: 
                          execute_query("select * from books"), 
                      wraplength = 200)
btn_books.grid(row = 5, column = 0, padx = 5, pady = 5)

btn_contract = tk.Button(root, text = "Таблица 'Контракты'", command = lambda: 
                             execute_query("select * from contract"),
                         wraplength = 200)
btn_contract.grid(row = 5, column = 1, padx = 5, pady = 5)

btn_creative_groups = tk.Button(root, text = "Таблица 'Творческие группы'",
                                command = lambda: 
...
btn_customers = tk.Button(root, text = "Таблица 'Заказчики'",
                          command = lambda: 
                              execute_query("select * from customers"),
                          wraplength = 200)
btn_customers.grid(row = 5, column = 3, padx = 5, pady = 5)

btn_orders = tk.Button(root, text = "Таблица 'Заказы'", command = lambda:
            execute_query("select * from orders"), wraplength = 200)
btn_orders.grid(row = 6, column = 0, padx = 5, pady = 5)

btn_published_books = tk.Button(root, text = "Таблица 'Опубликованные книги'",
                                command = lambda: 
                                    execute_query("select * from \
                                                  published_books"), 
                                wraplength = 200)
...
btn_writers = tk.Button(root, text = "Таблица 'Писатели'", command = lambda: 
                            execute_query("select * from writers"), 
                        wraplength = 200)
btn_writers.grid(row = 6, column = 2, padx = 5, pady = 5)

# Кнопки для выполнения готовых запросов
btn_select1 = tk.Button(root, text = "Вывод писателей, фамилия которых \
                        начинается с C", command = lambda: 
                            execute_query("select surname as Фамилия, \
                                          name as Имя, patronimic as Отчество \
...
btn_select2 = tk.Button(root, text = "Вывод авторов, написавших более 1 книги", 
                        ...
                        execute_query("select surname as Фамилия, name as Имя, \
                                       patronimic as Отчество from writers where \
                                       idPassport in (select writers from \
                                       published_books group by writers \
                                       having count(code) > 1 order \
                                       by writers)"),
                        wraplength = 150)
btn_select2.grid(row = 7, column = 1, padx = 5, pady = 5)

btn_select3 = tk.Button(root, text = "Вывод писателей по уникальной \
                        творческой группе", command = lambda: 
                            execute_query("select distinct creativeGroup \
                                           as Творческие_группы from writers \
                                           where creativeGroup is not null"),
                        wraplength = 150)
btn_select3.grid(row = 7, column = 2, padx = 5, pady = 5)

btn_select4 = tk.Button(root, text = "Вывод писателей с одинаковыми фамилиями",
                        command = lambda: execute_query("select * from writers \
                                                         where surname in \
...
btn_select5 = tk.Button(root, text = "Вывести писателей по номеру \
                        творческой группы",  command = lambda: 
                            execute_query("select * from writers where \
                                           creativeGroup like '2%'"),
                        wraplength = 150)
btn_select5.grid(row = 8, column = 1, padx = 5, pady = 5)

btn_select6 = tk.Button(root, text = "Вывод числа писателей однофамильцев",
                        command = lambda: 
                            execute_query("select count(*) as Число_записей \
                                          from writers where surname in \
                                          (select surname from writers group \
                                          by surname having count(surname) > 1) \
                                          order by surname"), wraplength = 150)
btn_select6.grid(row = 8, column = 2, padx = 5, pady = 5)

btn_select7 = tk.Button(root, text = "Вывод названий книг по дате заказов", 
                        command = lambda: 
                            execute_query("select name as Название_книги,\
                                          receiptDate as Дата_заказа from \
                                          books join orders on \
                                          (books.code = orders.code) order \
                                          by receiptDate"), wraplength = 150)
btn_select7.grid(row = 9, column = 0, padx = 5, pady = 5)

btn_select8 = tk.Button(root, text = "Заказ книг представителями", 
                        command = lambda: 
                            execute_query("select books.name as \
                                          Название_книги, circulation \
                                          as Тираж, orders.number as \
                                          Номер_заказа, agent as \
                                          Представитель from books \
                                          inner join orders on \
                                          (books.code = orders.code) \
...
btn_select9 = tk.Button(root, text = "Возвращаем номера", command = lambda: 
                           execute_query("select idcreative_groups as \
                                         Различные_номера from creative_groups \
                                          union (select idPassport from writers) \
                                          union (select number from contract)"), 
                        wraplength = 150)
