...
np.set_printoptions(
    suppress=True, formatter={"all": lambda x: str(x) + ","}, linewidth=9999
)
should_have_failed = 0
success = 0


def preprocess_game_state(
    game_state: str, remove_fallen_opp: bool = False
) -> tuple:
    """
    ...
    if len(opp) == 0:
        ...
        opp_array = [
            [float(x) for x in temp.split()] for temp in opp.split(";")[:-1]
        ]
...
def is_opponent_2far(
    game_state: tuple, threshold: float = 2.5, remove_fallen_opp: bool = False
) -> bool:
    """
...
def parse_game_state(
    ...
    if (
        len(opp_array) == 0 or kick_target[2] not in kick_type
    ) and skip_no_opp:
    ...
    closest_opp_angle_from_me, closest_opp_dist_from_me = (
        helper.find_target_angleand_dist(me, opp_array[closest_opp_index])
    )
    ...
    feature_array = np.array(
        [
            closest_opp_dist,
            opp_dist_2kick_line,
            angle_between_opp_and_kick_dir,
            num_opp_closer_than_dist,
            ang_between_opp_and_ball,
            ang2_turn_opp,
            angle2_turn_me,
            opp_ball_angle,
            opp_fallen,
            num_opp_within_kick_dir,
            closest_opp_angle_from_me,
            closest_opp_dist_from_me,
            dist_to_kick_pos_opp,
            mean_opp_dist,
            max_opp_dist,
            ball_angle,
            ball_dist,
...
def parse_kick_success_new(
    game_state: tuple, ball_movement, ez_kick_success: bool = False
) -> int:
    ...
    travel_dist = helper.get_dist_to(ball_before, target) - helper.get_dist_to(
        ball_after, target
    )
...
def parse(
    ...
    for index, line in enumerate(lines):
        ...
        try:
            ...
            feature_array, success = parse_game_state(
                game_state,
                n_features,
                kick_type=kick_type,
                remove_fallen_opp=remove_fallen_opp,
                use_direct_features=use_direct_features,
            )
        except:
            success = False
        ...
        if success:
            ...
            if ignore_self_failure:
                if labels[index] == 1 or not is_opponent_2far(
                    game_state, remove_fallen_opp=remove_fallen_opp
                ):
...
def batch_eval_accuracy(
    ...
    for i, file_name in enumerate(file_names):
        try:
            accuracyRate = classify(
                file_name=file_name,
                use_features=use_features,
                equal_class_size=False,
                use_all=False,
                batch=True,
                useCache=True,
                kick_type=kick_type,
...
def classify(
    ...
    features, labels = parse(
        file_name=file_name,
        use_cache=use_cache,
        ez_kick_success=False,
        kick_type=kick_type,
        ignore_self_failure=False,
        use_direct_features=True,
        n_features=8,
    )
    ...
    print(
        f"accuracy rate: {
            good / float(len(predict_labels))
        }, {good}"
    )
    print(
        f"true negative rate: {
            true_neg / float(len(predict_labels))
        }, {true_neg}"
    )
    print(
        f"true positive rate: {
            true_pos / float(len(predict_labels))
        }, {true_pos}"
    )
    print(
        f"false negative rate: {
            false_neg / float(len(predict_labels))
        }, {false_neg}"
    )
    print(
        f"false positive rate: {
            false_pos / float(len(predict_labels))
        }, {false_pos}"
    )
...
def draw_precision_recall_curve(
    file_name: str, test_labels: np.ndarray, prob_array: np.ndarray
) -> None:
...
def visualize_dribble_data(
    ...
    def parse_kick_data(kick: str) -> np.array:
        ...
        for index, line in enumerate(lines):
            ...
            init_time = float(init_time)
            x, immed_after_kick, x, after_kick, x, kick_info = (
                ball_movement.split(":")
            )
            kick_mode = int(kick_info.split()[0])
            if kick_mode != 2:
