...
PARAMHELP = (
    "specify parameter or keyword for rendering (multiple can be specified)"
)
...
def delete_cluster(args):
    ...
    for cluster in args.clusters:
        ...
        for infra_env in ai.list_infra_envs():
            ...
            associated_infra_envs = [
                f"{cluster}_infra-env",
                f"{cluster}-day2_infra-env",
            ]
            if (
                infra_env_name is not None
                and infra_env_name in associated_infra_envs
            ):
    ...
    for host in sorted(hosts, key=lambda x: x["requested_hostname"] or "zzz"):
        ...
        if "interfaces" in inventory and inventory["interfaces"]:
            if (
                ...
                and inventory["interfaces"][0]["ipv6_addresses"]
            ):
                ip = inventory["interfaces"][0]["ipv6_addresses"][0].split(
                    "/"
                )[0]
            if (
                "ipv4_addresses" in inventory["interfaces"][0]
                and inventory["interfaces"][0]["ipv4_addresses"]
            ):
...
def wait_hosts(args):
    ...
    ai.wait_hosts(
        args.infraenv, args.number, filter_installed=filter_installed
    )
...
def cli():
    ...
    parser.add_argument(
        "--staging",
        action="store_true",
        default=bool(os.environ.get("STAGING", "")),
    )
    parser.add_argument("-U", "--url", default=os.environ.get("AI_URL"))
    parser.add_argument("--token", default=os.environ.get("AI_TOKEN"))
    parser.add_argument(
        "--offlinetoken", default=os.environ.get("AI_OFFLINETOKEN")
    )
    subparsers = parser.add_subparsers(metavar="", title="Available Commands")

    bind_desc = "bind Object"
    bind_parser = subparsers.add_parser(
        "bind", description=bind_desc, help=bind_desc
    )
    bind_subparsers = bind_parser.add_subparsers(
        metavar="", dest="subcommand_bind"
    )

    create_desc = "Create Object"
    create_parser = subparsers.add_parser(
        "create", description=create_desc, help=create_desc, aliases=["add"]
    )
    create_subparsers = create_parser.add_subparsers(
        metavar="", dest="subcommand_create"
    )

    delete_desc = "Delete Object"
    delete_parser = subparsers.add_parser(
        "delete", description=delete_desc, help=delete_desc, aliases=["remove"]
    )
    ...
    delete_subparsers = delete_parser.add_subparsers(
        metavar="", dest="subcommand_delete"
    )

    download_desc = "Download Assets"
    download_parser = subparsers.add_parser(
        "download", description=download_desc, help=download_desc
    )
    download_subparsers = download_parser.add_subparsers(
        metavar="", dest="subcommand_download"
    )

    export_desc = "Export Object"
    export_parser = subparsers.add_parser(
        "export", description=export_desc, help=export_desc
    )
    export_subparsers = export_parser.add_subparsers(
        metavar="", dest="subcommand_export"
    )

    info_desc = "Info Object"
    info_parser = subparsers.add_parser(
        "info", description=info_desc, help=info_desc
    )
    info_subparsers = info_parser.add_subparsers(
        metavar="", dest="subcommand_info"
    )

    list_desc = "List Object"
    list_parser = subparsers.add_parser(
        "list", description=list_desc, help=list_desc, aliases=["get"]
    )
    list_subparsers = list_parser.add_subparsers(
        metavar="", dest="subcommand_list"
    )

    start_desc = "Start Object"
    start_parser = subparsers.add_parser(
        "start", description=start_desc, help=start_desc, aliases=["launch"]
    )
    start_subparsers = start_parser.add_subparsers(
        metavar="", dest="subcommand_start"
    )

    stop_desc = "Stop Object"
    stop_parser = subparsers.add_parser(
        "stop", description=stop_desc, help=stop_desc, aliases=["reset"]
    )
    stop_subparsers = stop_parser.add_subparsers(
        metavar="", dest="subcommand_stop"
    )

    unbind_desc = "Unbind Object"
    unbind_parser = subparsers.add_parser(
        "unbind", description=unbind_desc, help=unbind_desc
    )
    unbind_subparsers = unbind_parser.add_subparsers(
        metavar="", dest="subcommand_unbind"
    )

    update_desc = "Update Object"
    update_parser = subparsers.add_parser(
        "update", description=update_desc, help=update_desc, aliases=["patch"]
    )
    update_subparsers = update_parser.add_subparsers(
        metavar="", dest="subcommand_update"
    )

    wait_desc = "Wait Object"
    wait_parser = subparsers.add_parser(
        "wait", description=wait_desc, help=wait_desc
    )
    wait_subparsers = wait_parser.add_subparsers(
        metavar="", dest="subcommand_wait"
    )

    clustercreate_desc = "Create Cluster"
    clustercreate_epilog = None
    clustercreate_parser = create_subparsers.add_parser(
        "cluster",
        description=clustercreate_desc,
        help=clustercreate_desc,
        epilog=clustercreate_epilog,
        formatter_class=rawhelp,
    )
    clustercreate_parser.add_argument(
        "-P", "--param", action="append", help=PARAMHELP, metavar="PARAM"
    )
    clustercreate_parser.add_argument(
        "--paramfile", help="Parameters file", metavar="PARAMFILE"
    )
    ...
    clusterdelete_parser = delete_subparsers.add_parser(
        "cluster",
        description=clusterdelete_desc,
        help=clusterdelete_desc,
        epilog=clusterdelete_epilog,
        formatter_class=rawhelp,
    )
    clusterdelete_parser.add_argument(
        "clusters", metavar="CLUSTERS", nargs="*"
    )
    ...
    export_subparsers.add_parser(
        "cluster",
        parents=[clusterexport_parser],
        description=clusterexport_desc,
        help=clusterexport_desc,
        aliases=["clusters"],
    )

    clusterinfo_desc = "Info Cluster"
    clusterinfo_epilog = None
    clusterinfo_parser = info_subparsers.add_parser(
        "cluster",
        description=clusterinfo_desc,
        help=clusterinfo_desc,
        epilog=clusterinfo_epilog,
        formatter_class=rawhelp,
    )
    clusterinfo_parser.add_argument(
        "-f",
        "--fields",
        help="Display Corresponding list of fields," "separated by a comma",
        metavar="FIELDS",
    )
    clusterinfo_parser.add_argument(
        "-v", "--values", action="store_true", help="Only report values"
    )
    clusterinfo_parser.add_argument(
        "--full", action="store_true", help="Full output"
    )
    clusterinfo_parser.add_argument(
        "-p", "--preflight", action="store_true", help="Show preflight"
    )
    ...
    list_subparsers.add_parser(
        "cluster",
        parents=[clusterlist_parser],
        description=clusterlist_desc,
        help=clusterlist_desc,
        aliases=["clusters"],
    )
    ...
    list_subparsers.add_parser(
        "cluster-keyword",
        parents=[cluster_keywords_list_parser],
        description=cluster_keywords_list_desc,
        help=cluster_keywords_list_desc,
        aliases=["cluster-keywords"],
    )

    manifestscreate_desc = "Upload manifests to cluster"
    manifestscreate_epilog = None
    manifestscreate_parser = create_subparsers.add_parser(
        "manifest",
        description=manifestscreate_desc,
        help=manifestscreate_desc,
        epilog=manifestscreate_epilog,
        formatter_class=rawhelp,
        aliases=["manifests"],
    )
    manifestscreate_parser.add_argument(
        "--dir",
        "--directory",
        help="directory with stored manifests",
        required=True,
    )
    manifestscreate_parser.add_argument(
        "-o",
        "--openshift",
        action="store_true",
        help="Store in openshift folder",
    )
    ...
    clusterstart_parser = start_subparsers.add_parser(
        "cluster",
        description=clusterstart_desc,
        help=clusterstart_desc,
        epilog=clusterstart_epilog,
        formatter_class=rawhelp,
    )
    ...
    clusterstop_parser = stop_subparsers.add_parser(
        "cluster",
        description=clusterstop_desc,
        help=clusterstop_desc,
        epilog=clusterstop_epilog,
        formatter_class=rawhelp,
    )
    ...
    clusterupdate_parser.add_argument(
        "-P", "--param", action="append", help=PARAMHELP, metavar="PARAM"
    )
    clusterupdate_parser.add_argument(
        "--paramfile", help="Parameters file", metavar="PARAMFILE"
    ...
    update_subparsers.add_parser(
        "cluster",
        parents=[clusterupdate_parser],
        description=clusterupdate_desc,
    ...
    list_subparsers.add_parser(
        "event",
        parents=[eventslist_parser],
        description=eventslist_desc,
        help=eventslist_desc,
        aliases=["events"],
    )
    ...
    ignitiondiscoverydownload_parser.add_argument(
        "-p",
        "--path",
        metavar="PATH",
        default=".",
    ...
    ignitiondiscoverydownload_parser.set_defaults(
        func=download_discovery_ignition
    )
    download_subparsers.add_parser(
        "discovery-ignition",
        parents=[ignitiondiscoverydownload_parser],
    ...
    ignitiondownload_parser.add_argument(
        "-p",
        "--path",
        metavar="PATH",
        default=".",
    ...
    download_subparsers.add_parser(
        "ignition",
        parents=[ignitiondownload_parser],
    ...
    infraenvbind_parser.add_argument(
    ...
    bind_subparsers.add_parser(
        "infraenv",
        parents=[infraenvbind_parser],
        description=infraenvbind_desc,
        help=infraenvbind_desc,
    )

    infraenvcreate_desc = "Create Infraenv"
    infraenvcreate_epilog = None
    infraenvcreate_parser = create_subparsers.add_parser(
        "infraenv",
        description=infraenvcreate_desc,
        help=infraenvcreate_desc,
        epilog=infraenvcreate_epilog,
        formatter_class=rawhelp,
    )
    infraenvcreate_parser.add_argument(
        "-P", "--param", action="append", help=PARAMHELP, metavar="PARAM"
    )
    infraenvcreate_parser.add_argument(
        "--paramfile", help="Parameters file", metavar="PARAMFILE"
    ...
    infraenvdelete_parser = delete_subparsers.add_parser(
        "infraenv",
        description=infraenvdelete_desc,
        help=infraenvdelete_desc,
        epilog=infraenvdelete_epilog,
        formatter_class=rawhelp,
    )
    infraenvdelete_parser.add_argument(
        "infraenvs", metavar="INFRAENVS", nargs="*"
    )
    infraenvdelete_parser.set_defaults(func=delete_infra_env)

    infraenvinfo_desc = "Info Infraenv"
    infraenvinfo_epilog = None
    infraenvinfo_parser = info_subparsers.add_parser(
        "infraenv",
        description=infraenvinfo_desc,
        help=infraenvinfo_desc,
        epilog=infraenvinfo_epilog,
        formatter_class=rawhelp,
    )
    infraenvinfo_parser.add_argument(
        "-f",
        "--fields",
        help="Display Corresponding list of fields," "separated by a comma",
        metavar="FIELDS",
    ...
    list_subparsers.add_parser(
        "infraenv",
        parents=[infraenvlist_parser],
        description=infraenvlist_desc,
        help=infraenvlist_desc,
        aliases=["infraenvs"],
    )
    ...
    list_subparsers.add_parser(
        "infraenv-keyword",
        parents=[infraenv_keywords_list_parser],
        description=infraenv_keywords_list_desc,
        help=infraenv_keywords_list_desc,
        aliases=["infraenv-keywords"],
    )
    ...
    unbind_subparsers.add_parser(
        "infraenv",
        parents=[infraenvunbind_parser],
        description=infraenvunbind_desc,
        help=infraenvunbind_desc,
    )

    infraenvupdate_desc = "Update Infraenv"
    infraenvupdate_parser = argparse.ArgumentParser(add_help=False)
    infraenvupdate_parser.add_argument(
        "-P", "--param", action="append", help=PARAMHELP, metavar="PARAM"
    )
    infraenvupdate_parser.add_argument(
        "--paramfile", help="Parameters file", metavar="PARAMFILE"
    ...
    update_subparsers.add_parser(
        "infraenv",
        parents=[infraenvupdate_parser],
        description=infraenvupdate_desc,
        help=infraenvupdate_desc,
    )

    isocreate_desc = "Create iso"
    isocreate_epilog = None
    isocreate_parser = create_subparsers.add_parser(
        "iso",
    ...
    isocreate_parser.add_argument(
        "-m", "--minimal", action="store_true", help="Use minimal iso"
    )
    isocreate_parser.add_argument(
        "-P", "--param", action="append", help=PARAMHELP, metavar="PARAM"
    )
    ...
    isoinfo_epilog = None
    isoinfo_parser = info_subparsers.add_parser(
        "iso",
        description=isoinfo_desc,
        help=isoinfo_desc,
        epilog=isoinfo_epilog,
        formatter_class=rawhelp,
    )
    isoinfo_parser.add_argument(
        "-m", "--minimal", action="store_true", help="Use minimal iso"
    )
    isoinfo_parser.add_argument(
        "-s", "--short", action="store_true", help="Only print iso url"
    )
    isoinfo_parser.add_argument(
        "-P", "--param", action="append", help=PARAMHELP, metavar="PARAM"
    )
    ...
    initrddownload_parser.add_argument(
        "--path", metavar="PATH", default=".", help="Where to download asset"
    )
    initrddownload_parser.add_argument("infraenv", metavar="INFRAENV")
    initrddownload_parser.set_defaults(func=download_initrd)
    download_subparsers.add_parser(
        "initrd",
        parents=[initrddownload_parser],
        description=initrddownload_desc,
        help=initrddownload_desc,
    )

    infraenv_help = (
        "The infraenv associated to the cluster. Cluster name can also be used"
    )
    isodownload_desc = "Download Iso"
    isodownload_parser = argparse.ArgumentParser(add_help=False)
    isodownload_parser.add_argument(
        "-p",
        "--path",
        metavar="PATH",
        default=".",
    ...
    isodownload_parser.add_argument(
        "infraenv", metavar="INFRAENV", help=infraenv_help
    )
    isodownload_parser.set_defaults(func=download_iso)
    download_subparsers.add_parser(
        "iso",
        parents=[isodownload_parser],
    ...
    installconfigdownload_parser.add_argument(
        "--path", metavar="PATH", default=".", help="Where to download asset"
    )
    ...
    download_subparsers.add_parser(
        "installconfig",
        parents=[installconfigdownload_parser],
    ...
    kubepassworddownload_parser.add_argument(
        "--path", metavar="PATH", default=".", help="Where to download asset"
    )
    ...
    download_subparsers.add_parser(
        "kubeadmin-password",
        parents=[kubepassworddownload_parser],
    ...
    kubeconfigdownload_parser.add_argument(
        "--path", metavar="PATH", default=".", help="Where to download asset"
    )
    ...
    download_subparsers.add_parser(
        "kubeconfig",
        parents=[kubeconfigdownload_parser],
    ...
    delete_subparsers.add_parser(
        "host",
        parents=[hostdelete_parser],
        description=hostdelete_desc,
        help=hostdelete_desc,
    )

    hostinfo_desc = "Info Host"
    hostinfo_epilog = None
    hostinfo_parser = info_subparsers.add_parser(
        "host",
        description=hostinfo_desc,
        help=hostinfo_desc,
        epilog=hostinfo_epilog,
        formatter_class=rawhelp,
    )
    hostinfo_parser.add_argument(
        "-i", "--inventory", action="store_true", help="Report host inventory"
    )
    hostinfo_parser.add_argument(
        "-f",
        "--fields",
    ...
    list_subparsers.add_parser(
        "host",
        parents=[hostslist_parser],
        description=hostslist_desc,
        help=hostslist_desc,
        aliases=["hosts"],
    )
    ...
    unbind_subparsers.add_parser(
        "host",
        parents=[hostunbind_parser],
        description=hostunbind_desc,
        help=hostunbind_desc,
    )

    hostupdate_desc = "Update Host name and role"
    hostupdate_parser = argparse.ArgumentParser(add_help=False)
    hostupdate_parser.add_argument(
        "-P", "--param", action="append", help=PARAMHELP, metavar="PARAM"
    )
    hostupdate_parser.add_argument(
        "--paramfile", help="Parameters file", metavar="PARAMFILE"
    ...
    update_subparsers.add_parser(
        "host",
        parents=[hostupdate_parser],
        description=hostupdate_desc,
        help=hostupdate_desc,
    )
    ...
    list_subparsers.add_parser(
        "manifest",
        parents=[manifestslist_parser],
        description=manifestslist_desc,
        help=manifestslist_desc,
        aliases=["manifests"],
    )

    isopatch_desc = "Update Discovery Iso"
    isopatch_parser = argparse.ArgumentParser(add_help=False)
    isopatch_parser.add_argument(
        "-P", "--param", action="append", help=PARAMHELP, metavar="PARAM"
    )
    isopatch_parser.add_argument(
        "--paramfile", help="Parameters file", metavar="PARAMFILE"
    ...
    update_subparsers.add_parser(
        "installconfig",
        parents=[installconfigpatch_parser],
        description=installconfigpatch_desc,
        help=installconfigpatch_desc,
    )

    serviceinfo_desc = "Info Service"
    serviceinfo_epilog = None
    serviceinfo_parser = info_subparsers.add_parser(
        "service",
        description=serviceinfo_desc,
        help=serviceinfo_desc,
        epilog=serviceinfo_epilog,
        formatter_class=rawhelp,
    )
    ...
    hostswait_parser.add_argument(
        "-f", "--filter", action="store_true", help="Filter installed hosts"
    )
    hostswait_parser.add_argument(
        "-n",
        "--number",
        help="Number of nodes to wait for. Default to 3",
        type=int,
        default=3,
    )
    hostswait_parser.add_argument("infraenv", metavar="INFRAENV")
    hostswait_parser.set_defaults(func=wait_hosts)
    wait_subparsers.add_parser(
        "host",
        parents=[hostswait_parser],
        description=hostswait_desc,
        help=hostswait_desc,
        aliases=["hosts"],
    )
