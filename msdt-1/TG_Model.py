...
class TGModel(object):
    ...
    def build(self):
        ...
        embeddings = tf.Variable(tf.random_uniform(
            [self.config.vocab_size, self.config.hidden_size], -1.0, 1.0))
        ...
        output, last_state = tf.nn.dynamic_rnn(
            cell_mul, x, initial_state=initial_state)
        ...
        if self.action == 'train':
            ...
            onehot_labels = tf.one_hot(tf.reshape(
                self.labels_placeholder, [-1]), depth=self.config.vocab_size)
            masked_y = tf.boolean_mask(onehot_labels, y_mask, axis=0)
            loss = tf.nn.softmax_cross_entropy_with_logits(
                labels=masked_y, logits=masked_pred, name='loss')
            total_loss = tf.reduce_mean(loss)

            lr = tf.train.exponential_decay(
                self.config.lr, self.global_steps, 12600, 0.5)
            op = tf.train.AdamOptimizer(lr)
            grads, variables = zip(*op.compute_gradients(loss))
            grads, global_norm = tf.clip_by_global_norm(grads, 5)
            train_op = op.apply_gradients(
                zip(grads, variables), global_step=self.global_steps)
