...
def get_user_input():
    # Функция для ввода числа от игрока
    while True:
        try:
...
def main():
    # Основная программа без статистики
    welcome_message()
    play_game()
...
def count_attempts(attempts_list):
    # Подсчёт и вывод попыток угадать число
...
def play_game_with_stats():
    # Добавляем список для хранения количества попыток
    number = get_random_number()
    attempts = 0
    guessed = False
    attempts_list = []
...
def main_with_stats():
    # Основная программа с выводом статистики
    welcome_message()
...
def start_program():
    # Начало программы с выбором режима игры
