...
def multiply_matrices(matrix1, matrix2):
    ...
    if len(matrix1[0]) != len(matrix2):
        raise ValueError("The number of columns of the first matrix must be equal to the number of rows of the second one.")
    return [
...
def trace_matrix(matrix):
    ...
    if not is_square_matrix(matrix):
        raise ValueError("The trace can only be calculated for a square matrix.")
    return sum(matrix[i][i] for i in range(len(matrix)))

def determinant_matrix(matrix):
    """Recursive calculation of the determinant of a square matrix."""
    if not is_square_matrix(matrix):
        raise ValueError("The determinant can only be calculated for a square matrix.")
    if len(matrix) == 1:
        return matrix[0][0]
    if len(matrix) == 2:
    ...
    for col in range(len(matrix)):
        minor = [[row[i] for i in range(len(row)) if i != col] for row in matrix[1:]]
        determinant += ((-1) ** col) * matrix[0][col] * determinant_matrix(minor)
    return determinant
