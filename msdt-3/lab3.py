...
class Product:
    def __init__(self, card):
        ...
        self.rating = find_first_value(card, r'catalog-product__rating.*?b>([\d.]+).*?</b')
        self.is_sales_stopped = (find_first_value_without_group(card, r'Продажи прекращены') is not None)
        self.description = find_first_value(card, r'span>(.*?)</span')
        self.name = find_first_value_without_group(self.description, r'Стиральн.+?машина[a-zA-Z0-9 /-]+')
...
if __name__ == "__main__":
    ...
    product_list = find_first_value(page, r'class="products-list (.*?)pagination-container')
    cards = find_all_values(product_list, r'data-id="product".*?product-buy__price-wrap.*?</div></div>')
